...
/// Calls z.ai's usage API with a user-provided API key.
struct ZaiProvider: RegisterableProvider {
    static let key = "zai"
    ...
    static var descriptor: ProviderDescriptor {
        ProviderDescriptor(
            ...
            factory: { credential in
                ZaiProvider(apiKey: credential?.value ?? "")
            }
        )
    }
    ...
    init(
        apiKey: String,
        baseURL: URL = ProviderHTTP.hardcodedURL("https://api.z.ai"),

...
/// Calls Warp's usage API with a user-provided API token.
///
/// CodexBar strategy: API token from config or `WARP_API_KEY` / `WARP_TOKEN` env var.
struct WarpProvider: RegisterableProvider {
    static let key = "warp"
    static let displayName = "Warp"
    ...
    static var descriptor: ProviderDescriptor {
        ProviderDescriptor(
            ...
            factory: { credential in
                WarpProvider(apiKey: credential?.value ?? "")
            }
        )
    }
    ...
    init(
        apiKey: String,
        baseURL: URL = ProviderHTTP.hardcodedURL("https://app.warp.dev"),

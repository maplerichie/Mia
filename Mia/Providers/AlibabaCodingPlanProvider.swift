...
/// Reads an Alibaba Coding Plan web session from browser cookies or a manually-pasted
/// Cookie header and calls the Lingma/Alibaba Cloud coding-plan API.
///
/// CodexBar strategy: browser cookies or a Lingma API key pasted as the credential.
struct AlibabaCodingPlanProvider: RegisterableProvider {
    static let key = "alibabacoding"
    static let displayName = "Alibaba Coding Plan"
    ...
    static var descriptor: ProviderDescriptor {
        ProviderDescriptor(
            ...
            factory: { credential in
                AlibabaCodingPlanProvider(credential: credential?.value)
            }
        )
    }
    ...
    func fetchUsage() async throws -> UsageInfo? {
        ...
        var request = URLRequest(url: baseURL.appendingPathComponent("/api/lingma/usage"))

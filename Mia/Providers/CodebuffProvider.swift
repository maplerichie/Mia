import Foundation

/// Reads the local Codebuff credential from `~/.config/manicode/credentials.json`
/// and calls the Codebuff usage API.
///
/// CodexBar strategy: local file auth created by `codebuff login`, no CLI required.
struct CodebuffProvider: RegisterableProvider {
    static let key = "codebuff"
    static let displayName = "Codebuff"

    let requiresCredential = true

    static var descriptor: ProviderDescriptor {
        ProviderDescriptor(
            key: key,
            displayName: displayName,
            requiresCredential: true,
            credentialSource: .localFile(LocalFileSource(
                path: "~/.config/manicode/credentials.json",
                keyPath: ["api_key"],
                environmentOverride: nil
            )),
            factory: { credential in
                CodebuffProvider(apiKey: credential?.value)
            }
        )
    }

    private let apiKey: String?
    private let baseURL: URL
    private let client: HTTPClient
    private let now: @Sendable () -> Date

    init(
        apiKey: String?,
        baseURL: URL = ProviderHTTP.hardcodedURL("https://api.codebuff.com"),
        client: HTTPClient = URLSessionHTTPClient(),
        now: @escaping @Sendable () -> Date = { .now }
    ) {
        self.apiKey = apiKey
        self.baseURL = baseURL
        self.client = client
        self.now = now
    }

    func fetchPlan() async throws -> PlanInfo? {
        nil
    }

    func fetchUsage() async throws -> UsageInfo? {
        guard let apiKey, !apiKey.isEmpty else { throw ProviderError.missingCredential }

        var request = URLRequest(url: baseURL.appendingPathComponent("/v1/usage"))
        request.httpMethod = "GET"
        request.setValue("Bearer \(apiKey)", forHTTPHeaderField: "Authorization")
        request.setValue("application/json", forHTTPHeaderField: "accept")

        let (payload, data) = try await ProviderHTTP.fetchJSON(
            UsageResponse.self,
            request: request,
            client: client
        )

        return UsageInfo(
            used: Double(payload.used),
            limit: payload.limit.map(Double.init),
            unit: "credits",
            capturedAt: now(),
            rawJSON: String(data: data, encoding: .utf8)
        )
    }

    // MARK: - Decoding

    private struct UsageResponse: Decodable {
        let used: Int
        let limit: Int?
    }
}

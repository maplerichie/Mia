...
/// Calls Deepgram's balance/usage API with a user-provided API key.
struct DeepgramProvider: RegisterableProvider {
    static let key = "deepgram"
    ...
    static var descriptor: ProviderDescriptor {
        ProviderDescriptor(
            ...
            factory: { credential in
                DeepgramProvider(apiKey: credential?.value ?? "")
            }
        )
    }
    ...
    init(
        apiKey: String,
        baseURL: URL = ProviderHTTP.hardcodedURL("https://api.deepgram.com"),
    ...
    func fetchUsage() async throws -> UsageInfo? {
        ...
        var request = URLRequest(url: baseURL.appendingPathComponent("/v1/projects/balance"))

...
/// Calls a local Wayfinder gateway usage endpoint with an API key.
///
/// CodexBar strategy: API key from `WAYFINDER_API_KEY` env var or config,
/// defaulting to the local gateway at http://127.0.0.1:8787.
struct WayfinderProvider: RegisterableProvider {
    static let key = "wayfinder"
    static let displayName = "Wayfinder"
    ...
    static var descriptor: ProviderDescriptor {
        ProviderDescriptor(
            ...
            factory: { credential in
                WayfinderProvider(apiKey: credential?.value ?? "")
            }
        )
    }
    ...
    init(
        apiKey: String,
        baseURL: URL = ProviderHTTP.hardcodedURL("http://127.0.0.1:8787"),

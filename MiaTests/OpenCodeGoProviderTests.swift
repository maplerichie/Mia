...
final class OpenCodeGoProviderTests: XCTestCase {
    func testFetchUsageDecodesTokens() async throws {
        ...
        let provider = OpenCodeGoProvider(cookieHeader: "session=abc", client: stub)

        let usage = try await provider.fetchUsage()
        XCTAssertEqual(usage?.used, 700)
        XCTAssertEqual(usage?.limit, 7000)
    ...
    func testMissingCredentialThrows() async {
        let provider = OpenCodeGoProvider(cookieHeader: nil, client: StubHTTPClient(outcome: .response(status: 200, body: Data())))
        do {

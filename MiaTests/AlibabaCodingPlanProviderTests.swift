...
final class AlibabaCodingPlanProviderTests: XCTestCase {
    func testFetchUsageDecodesTokens() async throws {
        ...
        let provider = AlibabaCodingPlanProvider(credential: "session=abc", client: stub)
    ...
    func testMissingCredentialThrows() async {
        let provider = AlibabaCodingPlanProvider(credential: nil, client: StubHTTPClient(outcome: .response(status: 200, body: Data())))
        do {

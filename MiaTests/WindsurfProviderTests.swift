...
final class WindsurfProviderTests: XCTestCase {
    func testFetchUsageDecodesCredits() async throws {
        ...
        let provider = WindsurfProvider(cookieHeader: "session=abc", client: stub)
    ...
    func testMissingCredentialThrows() async {
        let provider = WindsurfProvider(cookieHeader: nil, client: StubHTTPClient(outcome: .response(status: 200, body: Data())))
        do {
